...
const int TOTAL_PACKETS = (TOTAL_PIXELS + 19) / 20;
...
void desifrado_cesar(uint8_t *data, int k) {
  for (int i = 0; i < TOTAL_PIXELS; i++) {
    data[i] = (data[i] - k + 256) % 256;
  }
}

void descifrado_asimetrico(uint8_t *data, uint64_t d, uint64_t n) {
  for (int i = 0; i < TOTAL_PIXELS; i++) {
    data[i] = (uint8_t)(mod_pow(data[i], d, n));
  }
}
...
void loop() {
  ...
  if (vw_get_message(buf, &buflen)) {
    ...
    int bytes_to_copy = min(DATA_BYTES_PER_PACKET, TOTAL_PIXELS - start_index);
    ...
    if (!is_index_received(start_index)) {
      memcpy(&received_image[start_index], pixel_data, bytes_to_copy);
    ...
    if (imagen_received()) {
      ...
      //descifrado_asimetrico(received_image, D, N);
      desifrado_cesar(received_image,3);
